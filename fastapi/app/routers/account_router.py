...
@router.post(
    "/v1/accounts/sign-up",
    response_model=account_response.Jwt,
...
async def sign_up(
    request: account_request.SignUp,
    db: Session = Depends(settings.get_db),
) -> account_response.Jwt:
    """
    新規登録
    """
    ...
    return account_response.Jwt(access_token=access_token, refresh_token=refresh_token)
...
async def sign_in(
    request: account_request.SignIn,
    db: Session = Depends(settings.get_db),
) -> account_response.Jwt:
    """
    ログイン
...
async def refresh_token(
    request: account_request.RefreshToken,
    db: Session = Depends(settings.get_db),
) -> account_response.Jwt:
    """
    トークンのリフレッシュ
...
@router.get(
    "/v1/accounts/me",
    response_model=account_response.Account,
...
async def me(
    ...
    自分の情報を取得
    """
    return account_response.Account.model_validate(auth_user)
